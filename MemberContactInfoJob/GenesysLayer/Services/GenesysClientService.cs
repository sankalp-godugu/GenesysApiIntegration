...
namespace GenesysContactsProcessJob.GenesysLayer.Services
{
    ...
    public class GenesysClientService : IGenesysClientService
    {
        ...
        public async Task<IEnumerable<GetContactsResponse>> GetContactsFromContactList(ILogger logger)
        {
            return await GetContactsToContactList(logger);
        }
        ...
        public async Task<IEnumerable<AddContactsResponse>> AddContactsToContactList(IEnumerable<PostDischargeInfoPlusGenesys> contactsToAdd, ILogger logger)
        {
            ...
            StringContent content = GetRequestBodyForGenesys(contactsToAdd, logger);
        ...
        public async Task<IEnumerable<UpdateContactsResponse>> UpdateContactsInContactList(IEnumerable<PostDischargeInfoPlusGenesys> contactsToUpdate, ILogger logger)
        {
            ...
            StringContent content = GetRequestBodyForGenesys(contactsToUpdate, logger);
        ...
        /// <param name="logger">Logger.<see cref="ILogger"/></param>
        /// <returns>Returns 1 for success.</returns>
        public async Task<long> DeleteContactsFromContactList(IEnumerable<PostDischargeInfoPlusGenesys> contactsToDelete, ILogger logger)
        {
            // Gets the request body for the zendesk API request.
            string queryArgs = GetRequestQueryForGenesys(contactsToDelete, logger);
        ...
        private StringContent GetRequestBodyForGenesys(IEnumerable<PostDischargeInfoPlusGenesys> contactsToProcess, ILogger logger)
        {
            try
            {
                var mapper = MapperConfig.InitializeAutomapper();
                IEnumerable<AddContactsRequest> gcr = mapper.Map<List<AddContactsRequest>>(contactsToProcess);
        ...
        private string GetRequestQueryForGenesys(IEnumerable<PostDischargeInfoPlusGenesys> contactsToDelete, ILogger logger)
        {
            try
            {
                string contactIds = "";
        ...
        private async Task<IEnumerable<GetContactsResponse>> GetContactsToContactList(ILogger logger)
        {
            // Gets the Genesys http client.
